...
\begin{document}
    ...
    \defitem{Пусть положительный ряд $\sum a_n$ сходится и $r_n$ его частичная сумма. Докажите, что ряд $\sum (\sqrt{r_{n}} - \sqrt{r_{n+1}})$ также сходится, причём медленнее, чем ряд $\sum a_{n+1}$.}
    ...
    \defitem{Докажите, что всякий раз, когда признак Даламбера даёт ответ на вопрос о сходимости ряда, то радикальный признак Коши даёт (тот же) ответ на этот вопрос.}
        ...
        Покажем, из чего следует неравенство выше.
        
        Пусть $q = \overline{\lim}\dfrac{a_{n+1}}{a_n}$, $p = \overline{\lim}{\sqrt[n]{a_n}}$.
        Предположим, что $p < q$.
        Тогда для любого $\varepsilon$ существуют такие номера ${n_k}$, что $\sqrt[n_k]{a_{n_k}} \leqslant q - \varepsilon$.
        Тогда $a_{n_k} \geqslant (q - \varepsilon)^{n_k}$.
        Кроме того, существует $n_0$, такое, что $\dfrac{a_{n+1}}{a_{n}} \leqslant p + \varepsilon$, при $n > n_0: a_{n_0 + m} \leqslant a_{n_0}(p + \varepsilon)^m$
        
        Получаем $(q - \varepsilon)^{n_k} \leqslant a_{n_k} \leqslant a_{n_0}(p + \varepsilon)^{n_k - n_0}$.
        
        При всех $k = 1, 2, \dots$, у нас $\dfrac{a_{n_0}}{(p + \varepsilon)^{n_0}} \geqslant \dfrac{((q - \varepsilon)}{(p + e))^{n_k}}$.
        Но у нас при малых $\varepsilon$ наблюдается $\dfrac{(q-\varepsilon)}{(p+\varepsilon)} > 1$, а значит предположение неверно, $p \geqslant q$.
