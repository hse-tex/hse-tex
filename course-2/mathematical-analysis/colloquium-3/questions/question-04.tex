...
\begin{proof} $\\$
        ...
        \begin{align*}
                ...
                = \sum\limits_{k=1}^n\ \int\limits_{x_{k-1}}^{x_k} f(x)\Big(g(x)+\underbrace{g(x_{k-1})-g(x_{k-1})}_{\text{прибавили и вычли}}\Big)dx=\\
                &=\sum\limits_{k=1}^n\ \int\limits_{x_{k-1}}^{x_k} \Big(f(x)\Big(g(x)-g(x_{k-1})\Big)\Big)dx + \sum\limits_{k=1}^n\ \int\limits_{x_{k-1}}^{x_k} f(x)\cdot\underbrace{g(x_{k-1})}_{\mathclap{\text{не зависит от x}}}dx=\\
