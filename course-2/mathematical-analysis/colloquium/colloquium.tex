...
coffecat46 | \href{https://github.com/coffecat46}{github}}
\date{Версия от \today}
\maketitle
...
\input{assets/lecture05/common_refinement.tex}
% ПОФИКСИТЬ ФОТО
...
\begin{center}
    \input{assets/lecture07/example_for_7_2}
\end{center}
...
\begin{itemize}
    ...
    \item \textit{Замкнутость.} Пойдем от противного. $K$ — компакт, тогда возьмем $\{B_{\frac{\delta(x)}{2}}(x)\}_{x\in K}$ — покрытие открытыми шарами, где $\delta(x)=\rho(x,x_0)$. $x_0$ — предельная точка, которая $\notin K$ (или же $\in \mathbb{R}^n\setminus K$)
...
\begin{equation*}
    \us(f, \T) \le \us(f, \tilde{\T}) \underset{\text{по 3.9.1}}{\le} \os (f, \tilde{\T}) \le \os (f, \T)
\end{equation*}\qed
...
\proof $\forall \T_1, \T_2$ рассмотрим $\tilde{\T} = \T_1 \cap \T_2$, тогда по 3.9.2:
\begin{equation*}
...
\begin{center}
    \input{assets/lecture08/example_for_8_1}
\end{center}
