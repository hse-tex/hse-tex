...
\begin{document}
	...
	\defitem{Формулы Лагранжа и Коши.}
		...
		\begin{namedtheorem}[Коши. Обобщает формулу конечных приращений Лагранжа.]
			Пусть функции $f(x)$ и $g(x)$ непрерывны на отрезке $[a, b]$ и дифференцируемы на интвервале $(a, b)$, причем $g'(x) \neq 0$ при всех $x \in (a, b)$. Тогда в этом интервале существует точка $x = \xi$ такая, что
	...
	\defitem{Многочлен Тейлора и формула Тейлора для функций одной переменной.}
		...
		Предположим пока, что $x_0 = 0$. Тогда $P_n(x) = c_0 + c_1x + c_2x^2 + \dots + c_nx^n$. $P_n(0) = c_0$, а $P_n'(x) = c_1 + 2c_2x + \dots + nc_nx^{n - 1}$, из чего следует, что $c_1 = P_n'(0)$. По аналогии можно получить, что $c_2 = \dfrac{P_n''(0)}{2!}, \dots, c_n = \dfrac{P_n^{(n)}}{n!}$. Т.е. получаем, что $P_n(x) = P_n(0) + \dfrac{P_n'(0)}{1!}x + \dots + \dfrac{P_n^{(n)}(0)}{n!} x^n$.
	...
	\defitem{Формулы Маклорена для основных элементарных функций.}
		...
		\begin{enumerate}[leftmargin=*]
			...
			\item
			$\sin (x) = x - \dfrac{x^3}{3!} + \dfrac{x^5}{5!} - \dots + (-1)^{n - 1} \cdot \dfrac{x^{2n - 1}}{(2n - 1)!} + \os(x^{2n - 1})$

			\item
			$\cos (x) = 1 - \dfrac{x^2}{2!} + \dfrac{x^4}{4!} + \dots + (-1)^{n} \dfrac{x^{2n - 2}}{(2n - 2)!} + \os(x^{2n - 2})$
	...
	\proofitem{Геометрический смысл дифференциала. [На коллоквиуме будет отсутствовать]}

		Дифференциал функции численно равен приращению ординаты касательной, проведенной к графику функции $y = f(x)$ в данной точке, когда аргумент $x$ получает приращение $\Delta x$.

		\href{https://lms2.sseu.ru/courses/eresmat/metod/met1/razdmet1_4/parmet1_4_2.htm}{Подробнее тут}

	\proofitem{Инвариантность формы первого дифференциала. [На коллоквиуме будет отсутствовать]}
	\proofitem{Производные и дифференциалы высших порядков функции одной переменной в точке. [На коллоквиуме будет только производная высших порядков]}
	...
	\proofitem{Правило Лопиталя.}
		\begin{enumerate}[leftmargin=*]
			...
			Докажем теорему для случая, когда пределы равны 0
			...
			\item
			Докажем теорему для случая, когда пределы равны $\infty$. Условия:
			...
			\begin{namedtheorem}[Лопиталя (второе правило)]
				...
				\begin{enumerate}
					...
					\item
					$\lim_{x \to a + 0}$ представляет собой неопределенность вида $\dfrac{\infty}{\infty}$
				...
				то $\lim_{x \to a + 0} \dfrac{f(x)}{g(x)} = A$
			\end{namedtheorem}

			\begin{proof}
				TODO(): почемутакмного, точно ли это было на лекции? 
				\href{http://hijos.ru/izuchenie-matematiki/mat-analiz-11-klass/15-pravila-lopitalya/}{Пруф}
			\end{proof}
		\end{enumerate}
	...
	\proofitem{Достаточные условия локального экстремума для функции одной переменной. [На коллоквиуме будет отсутствовать]}
	\proofitem{Выпуклые (вогнутые) функции одной переменной. [На коллоквиуме будет отсутствовать]}
	\proofitem{Достаточные условия выпуклости (вогнутости). [На коллоквиуме будет отсутствовать]}
	\proofitem{Точки перегиба.}
	\proofitem{Необходимые и достаточные условия для точки перегиба. [На коллоквиуме будет отсутствовать]}
	\proofitem{Асимптоты графика функции одной переменной. [На коллоквиуме будет отсутствовать]}
