...
\author{Игорь Балюк, 193 группа, @lodthe}
...
\begin{document}
	...
	Ориентировочная дата проведения: 11.11.2019

	\subsubsection{В обязательный минимум входят}
	\subsubsubsection{Определение предела числовой последовательности}
	Число $a$ называется пределом числовой последовательности $\{x_n\}$, если
	\begin{equation*}
		\forall \eps > 0, \exists N(\eps): \forall n \geqslant N \implies |x_n - a| < \eps
	\end{equation*}

	\subsubsubsection{Определение точной верхней и нижней грани}
	Верхняя (нижняя) грань числового множества $X$ --- число $a$ такое, что $\forall x \in X \implies x \leqslant (\geqslant) a$
	...
	\[\begin{gathered}
		a = \sup X \iff (\forall x \in X \implies x \leqslant a) \land (\nexists b: \;b < a, \forall x \in X \implies x \leqslant b) \\
		a = \inf X \iff (\forall x \in X \implies x \geqslant a) \land (\nexists b: \;b > a, \forall x \in X \implies x \geqslant b) \\
	\end{gathered}\]

	\subsubsubsection{Определение бесконечно малой и бесконечно большой последовательности}
	Последовательность $\{x_n\}$ называется 
	\begin{itemize}
		\item
		бесконечно малой последовательностью, если 

		$\forall \eps > 0 \; \exists N(\eps): \; \forall n \geqslant N \implies |x_n| < \eps|$

		\item
		бесконечно большой последовательностью, если 

		$\forall A > 0 \; \exists N(A): \; \forall n \geqslant N \implies |x_n| > A$
	\end{itemize}

	\subsubsubsection{Определение предела функции в точке и на бесконечности по Коши и по Гейне}
	\begin{itemize}
		\item
	...
	\subsubsubsection{Определение фундаментальной последовательности, критерий Коши сходимости последовательности}
	...
	\subsubsubsection{Первый и второй замечательный пределы}
	\begin{enumerate}
		\item
		\begin{equation*}
			\lim_{x \to 0} \dfrac{\sin x}{x} = 1
	...
	\subsubsubsection{Таблица производных элементарных функций}
	\begin{align*}
		&(C)' = 0 \\
		&(x^a)' = a \cdot x^{a - 1} \\
		&(a^x)' = a^x \ln a \\
	...
	\subsubsection{Основные понятие и теоремы (с доказательствами)}
	\subsubsubsection{Числовые последовательности. Примеры.}
	...
	\subsubsubsection{Понятие предела последовательности.}
	Число $a$ называется пределом числовой последовательности $\{x_n\}$, если
	\begin{equation*}
		\forall \eps > 0, \exists N(\eps): \forall n \geqslant N \implies |x_n - a| < \eps
	\end{equation*}

	\subsubsubsection{Ограниченные и неограниченные последовательности.} 
	\begin{itemize}
		\item
		...
		\begin{equation*}
			\{x_n\} \text{ ограниченная сверху } \iff \exists M \in X: \; \forall N \implies x_n \leqslant M
		\end{equation*}
		...
		\begin{equation*}
			\{x_n\} \text{ ограниченная снизу } \iff \exists m \in X: \; \forall N \implies x_n \geqslant m
		\end{equation*}
		...
		\begin{equation*}
			\{x_n\} \text{ ограниченная } \iff \exists M, m \in X: \; \forall N \implies m \leqslant x_n \leqslant M
		\end{equation*}

		\item
		Неограниченная последовательность — это последовательность, которая не является ограниченной.
		\begin{equation*}
			\{x_n\} \text{ неограниченная } \iff \forall M, m \in X: \; \exists N \implies (x_N < m) \lor (x_N > M)
		\end{equation*}
		...
		\begin{equation*}
			\{x_n\} \text{ ограниченная } \iff \exists A \in \RR: \; \forall N \implies |x_N| \leqslant A
		\end{equation*}
	\end{itemize}

	\subsubsubsection{Теорема об ограниченности сходящейся последовательности.}
	...
	\begin{proof}
		...
		\begin{equation*}
			\forall \eps > 0 \; \exists N: \; \forall n \gs N \implies |x_n - a| < \eps
		\end{equation*}
		Положим $A = \max\{|x_1|, \dots, |x_N|, |a - \eps|, |a + \eps|\}$. Тогда, $\forall n \in \NN: \; |x_n| \ls A$.
	\end{proof}
	
	\subsubsubsection{Теорема о единственности предела сходящейся последовательности.}
	\begin{theorem*}
	...
	\begin{proof}
		 ...
		 \begin{equation*}
		 	\begin{cases}
		 		a < b, \\
		 		\forall \eps > 0, \exists N_1(\eps): \forall n \geqslant N_1(\eps) \implies |x_n - a| < \eps, \\
		 		\forall \eps > 0, \exists N_2(\eps): \forall n \geqslant N_2(\eps) \implies |x_n - b| < \eps, \\
		 	\end{cases}
		 \end{equation*}
		 Положим $\eps = \dfrac{b - a}{2}$ и $N = max\{N_1(\eps), N_2(\eps)\}$. Тогда, $\forall n \geqslant N \implies  |x_n - a| < \eps \; \land \; |x_n - b| < \eps$. Возьмём $n \geqslant N$, тогда,
		 \begin{equation*}
		 	b - a = |b - a| = |b - x_n + x_n - a| \geqslant |x_n - b| + |x_n - a| < \dfrac{b - a}{2} + \dfrac{b - a}{2} = b - a
		 \end{equation*}
		 Пришли к противоречию ($b - a < b - a$).
	\end{proof}

	\subsubsubsection{Теорема о переходе к пределу в неравенствах.}
	\begin{theorem*}
		Пусть $\lim\limits_{n \to \infty} x_n = a, \lim\limits_{n \to \infty} y_n = b$. Если $a < b$, то
		$\exists N: \; \forall n \geqslant N \implies x_n < y_n$.
	\end{theorem*}
	\begin{proof}
		Из аксиомы полноты $\exists c: \; a < c < b$. 
		\[\begin{gathered}
			\forall \eps_1 > 0 \; \exists N_1: \; \forall n \geqslant N_1 \implies |x_n - a| < \eps_1 \\
			\forall \eps_2 > 0 \; \exists N_2: \; \forall n \geqslant N_2 \implies |y_n - b| < \eps_2 \\
		\end{gathered}\]
		Тогда, $\forall n \geqslant N_1 \implies |x_n - a| < c - a$ и $\forall n \geqslant N_2 \implies |y_n - b| < b - c$. Отсюда $\forall n \geqslant \max \{N_1, N_2\} \implies x_n < c - a + a = c = c - b + b < y_n$.
	\end{proof}

	\subsubsubsection{Теорема о вынужденном пределе (Теорема о двух милиционерах).}
	\begin{theorem*}
		Если $\forall n \in \NN: x_n \leqslant y_n \leqslant z_n$ и $\exists \lim\limits_{n \to \infty} x_n = a = \lim\limits_{n \to \infty} z_n$, тогда $\lim\limits_{n \to \infty} y_n = a$.
	\end{theorem*}
	\begin{proof}
		Из определения предела $\{x_n\}$, $\forall \eps > 0 \; \exists N_1: \; \forall n \geqslant N_1 \implies |x_n - a| < \eps \iff a - \eps < x_n < a + \eps$. Аналогично для предела $\{z_n\}$, $\forall \eps > 0 \; \exists N_2: \; \forall n \geqslant N_2 \implies |z_n - a| < \eps \iff a - \eps < z_n < a + \eps$. Тогда, 
		$\forall n \geqslant \max \{N_1, N_2, N\} \implies \\a - \eps < x_n \leqslant y_n \leqslant z_n < a + \eps \implies \lim\limits_{n \to \infty} y_n = a$.
	\end{proof}

	\subsubsubsection{Теорема о сходимости монотонных ограниченных последовательностей.}
	\begin{theorem*}
	...
	\begin{proof}
		...
		\begin{equation*}
			\forall \eps > 0 \; \exists N: \; \forall n \gs N \implies S - \eps < x_N \ls x_n \ls S \implies |x_n - S| < \eps
		\end{equation*}
	\end{proof}
	Аналогичное доказательство для ограниченной невозрастающей ч.п.

	\subsubsubsection{Определение числа е.}
	\begin{equation*}
	...
	\begin{proof}
		...
		Далее докажем, что последовательность $\{e_n\}$ является ограниченной. Действительно, первый член любой монотонно возрастающей последовательности является ее наибольшей нижней границей и, таким образом, $e_n \gs 2 \; \forall n \in \NN$.
	...
	\subsubsubsection{Бесконечно малые последовательности.}
	Последовательность $a_n$ называется бесконечно малой, если $\lim\limits_{n \to \infty} a_n = 0$.

	\subsubsubsection{Связь со сходящимися последовательностями.}
	...
	\subsubsubsection{Арифметические свойства бесконечно малых и сходящихся последовательностей.} 
	Пусть $\{\alpha_n\}$ --- бесконечно малая числовая последвательность.
	\begin{itemize}
		\item
		$\{\alpha_n\}$ ограничена
		\begin{proof}
			Как известно, $\forall \eps > 0 \; \exists N: \; \forall n > N \implies |\alpha_n| < \eps$. Значит, для всех $n > N$ доказано. Но $\forall n < N \implies \alpha_n \ls \max\{|\alpha_1|, |\alpha_2|, \dots, |\alpha_{N - 1}|\}$. Тогда выберем $\eps = 1, A = \max\{|\alpha_1|, |\alpha_2|, \dots, |\alpha_{N - 1}|, 1\} \implies \forall n \in \NN, |\alpha_n| \ls A$.
		\end{proof}

		\item
 		Если $\{y_n\}$ ограничена, то $\{y_n \cdot \alpha_n\}$ --- бесконечно малая.
 		\begin{proof}
 			$\{\alpha_n\}$ --- бесконечно малая, поэтому $\forall \eps > 0 \; \exists N: \; \forall n \gs N \implies |\alpha_n| < \dfrac{\eps}{A}$. Ввиду ограниченности $\{y_n\}, \exists A: \; \forall n \in \NN \implies |y_n| \ls A$. Но тогда $\{y_n \cdot \alpha_n\}: \forall \eps > 0 \; \exists N: \; \forall n \gs N \implies |y_n \cdot \alpha_n| < \dfrac{\eps}{A} \cdot A = \eps$.
 		\end{proof}
 		...
 		\begin{proof}
 			\[\begin{gathered}
 				\forall \eps > 0 \; \exists N_1: \forall n \gs N_1 \implies |\alpha_n| < \frac{\eps}{2}
 				\text{ и }
 				\forall \eps > 0 \; \exists N_2: \forall n \gs N_2 \implies |\beta_n| < \frac{\eps}{2} \\
 				\text{Тогда при } N = \max\{N_1, N_2\} \implies \forall n \gs N \implies
 				|\alpha_n \pm \beta_n| \ls |\alpha_n| + |\beta_n| < \frac{\eps}{2} + \frac{\eps}{2} = \eps
 			\end{gathered}\]
 			Аналогично для произведения:
 			\[\begin{gathered}
 				\forall \eps > 0 \; \exists N_1: \forall n \gs N_1 \implies |\alpha_n| < \frac{1}{\eps}
 				\text{ и }
 				\forall \eps > 0 \; \exists N_2: \forall n \gs N_2 \implies |\beta_n| < \eps^2 \\
 				\text{Тогда при } N = \max\{N_1, N_2\} \implies \forall n \gs N \implies
 				|\alpha_n \pm \beta_n| \ls |\alpha_n| \cdot |\beta_n| < \frac{1}{\eps} \cdot \eps^2 = \eps
 			\end{gathered}\]
 		\end{proof}
	\end{itemize}

	\subsubsubsection{Бесконечно большие последовательности, их связь с бесконечно малыми.}
	\begin{itemize}
		\item
		Если $\{x_n\}$ --- бесконечно малая и $\forall n \in \NN \implies x_n \neq 0$, то $\{\dfrac{1}{x_n}\}$ --- бесконечно большая.
		\begin{proof}
			$\forall \eps > 0 \; \exists N: \; \forall n \gs N \implies |x_n| < \eps \iff \dfrac{1}{|x_n|} < \dfrac{1}{\eps} = A$
		\end{proof}

		\item
		Если $\{x_n\}$ --- бесконечно большая и $\forall n \in \NN \implies x_n \neq 0$, то $\{\dfrac{1}{x_n}\}$ --- бесконечно малая.
		\begin{proof}
			$\forall A > 0 \; \exists N: \; \forall n \gs N \implies |x_n| > A \iff \dfrac{1}{|x_n|} < \dfrac{1}{A} = \eps$
		\end{proof}
	\end{itemize}

	\subsubsubsection{Арифметические свойства для последовательностей, имеющих конечные и бесконечные пределы.} 
	...
	\begin{proof}
		...
		\end{gathered}\]
		\begin{lemma}
			Пусть $\exists \lim\limits_{n \to \infty} y_n \neq 0$. Тогда $\exists r > 0: \exists N \in \NN: \forall n \gs N \implies |y_n| > r > 0$.
		\end{lemma}
		\begin{proof}
			$\forall \eps > 0 \exists N: \forall n \gs N \implies |y_n - b| < \eps \iff b - \eps < |y_n| < b + \eps$. Пусть $\eps = \dfrac{b}{2}$, тогда $r < \dfrac{b}{2} < y_n < \dfrac{3b}{2}$.
		\end{proof}
		...
		\end{gathered}\]
		По лемме $\left|\frac{1}{y_n \cdot b}\right| \ls max\left\{\left|\frac{1}{y_1 \cdot b}\right|, \dots, \left|\frac{1}{y_N \cdot b}\right|, \frac{1}{rb} \right\} \implies \{\frac{1}{y_n \cdot b}\}$ ограничена. Но тогда имеем произведение бесконечно малой и ограниченной последовательностей, значит, $\{\dfrac{x_n}{y_n} - \dfrac{a}{b}\}$ --- бесконечно малая.
	\end{proof}

	\subsubsubsection{Неопределенности.} 
	...
	\subsubsubsection{Определение подпоследовательности.} 
	...
	\subsubsubsection{Теорема Больцано-Вейерштрасса.} \label{ssss:bolzano}
	\begin{theorem*}
		Из любой ограниченной последовательности можно выделить сходящуюся подпоследовательность.
	\end{theorem*}
	\begin{proof}
		Для понимания происходящего следует ознакомиться с \ref{ssss:segments} Системой стягивающихся отрезков (ССО)

		$\{x_n\}$ ограничена $\implies \exists [a, b]: \forall n \in N \implies a \ls x_n \ls b$. Поделим $[a; b]$ на две равные части. Хотя бы одна из частей (пусть это $[a_1; b_1]$) содержит бесконечно много элементов $\{x_n\}$.
	...
	\subsubsubsection{Критерий Коши сходимости последовательности.} 
	\underline{Критерий Коши}: Для того, чтобы последовательность $\{x_n\}$ сходилась, необходимо и достаточно, чтобы она была фундаментальной.

	Последовательность называется фундаментальной, если 

	$\forall \eps > 0 \; \exists N: \forall n, m \gs N: |x_n - x_m| < \eps$
	\begin{proof}
		...
		\begin{itemize}
			...
			\begin{equation*}
				\forall \eps > 0: \; \exists N: \; \forall p \gs N \implies |x_p - a| < \eps
			\end{equation*}
			Поскольку $\eps$ произвольное, можно взять вместо него $\dfrac{\eps}{2}$
			\[\begin{gathered}
				p = m \gs N \implies |x_m - a| < \frac{\eps}{2} \\
				p = n \gs N \implies |x_n - a| < \frac{\eps}{2} \\
				|x_n - x_m| = |x_n - a + a - x_m| \ls |x_n - a| + |a - x_m| < \frac{\eps}{2} + \frac{\eps}{2} = \eps
			\end{gathered}\]
			...
			\[\begin{gathered}
				|x_n| = |(x_n - x_N) + x_N| \ls \underbrace{|x_n - x_N|}_{\ls \eps} + |x_N| \ls 1 + |x_N| \\
				\forall n \gs N \implies |x_n| \ls (1 + |x_N|) \; = const = A \implies |x_n| \ls A \\
				A = \max\{1 + |x_N|; |x_1|; |x_2|; \dots; |x_N|\} \\
				\forall n \gs N \implies |x_n| \ls A
			\end{gathered}\]
			По теореме \ref{ssss:bolzano} Больцано-Вейерштрасса, так как $\{x_n\}$ --- ограниченная, $\{x_n\}$ имеет сходящуюся подпоследовательность $\{x_{n_k}\}$.
			...
			\begin{equation*}
				\forall \eps > 0: \; \exists N_1: \; \forall p \gs N_1 \implies |x_p - a| < \frac{\eps}{2}
			\end{equation*}
			Так как $\{x_{n_k}\}$ сходящаяся:
			\[\begin{gathered}
				\lim_{k \to \infty} x_{n_k} = a: \; \forall \eps > 0 \; \exists N_2: \; \forall n_k \gs n_{N_2} \implies |x_{n_k} - a| < \frac{\eps}{2} \\
				\forall \eps > 0: \; |x_n - a| = |(x_n - x_{n_k}) + (x_{n_k} - a)| \ls |x_n - x_{n_k}| + |x_{n_k} - a| < \frac{\eps}{2} + \frac{\eps}{2} = \eps \\
				\text{Возьмём } N = \max\{N_1, N_2\}: \; \forall \eps > 0 \; \exists N: \; \forall n \gs N \implies |x_n - a| < \eps
			\end{gathered}\]
			Достаточность доказана.
		\end{itemize}
	\end{proof}

	\subsubsubsection{Определение предела функции в точке по Коши и по Гейне.} 
	\begin{itemize}
		\item
	...
	\subsubsubsection{Теорема об эквивалентности этих определений.} 
	\begin{itemize}
		\item
		...
		\begin{equation*}
			\forall \delta > 0 \; \exists N: \; \forall n \gs N \implies |x_n - a| < \delta
		\end{equation*}
		...
		\begin{equation*}
			\exists \eps_0: \; \forall \delta > 0: \; \exists x: \; 0 < |x - a| < \delta: \; |f(x) - A| \gs \eps_0
		\end{equation*}
		Так как $\delta$ может быть любым, можно выбрать последовательность $\{\delta_n\} = \{\dfrac{1}{n}\}$, а соответствующие значения $x$ будем обозначать как $x_n$. Тогда $0 < |x_n - a| < \delta_n = \dfrac{1}{n}$, и $|f(x_n) - A| \gs \eps_0$. Отсюда следует, что последовательность $\{x_n\}$ является подходящей, но при этом число $A$ не является пределом функции $f(x)$ в точке $a$ (по Гейне). Пришли к противоречию.
	\end{itemize}

	\subsubsubsection{Односторонние пределы, их связь с двусторонними. Пределы функции в бесконечности.}
	...
	\subsubsubsection{Неопределенности. Теоремы о переходе к пределу в неравенствах, о вынужденном пределе. [Не пройдено]}
	\subsubsubsection{Теорема о пределе сложной функции. [Не пройдено]} 
	\subsubsubsection{Первый и второй замечательные пределы. [Не пройдено]} 
	\subsubsubsection{Сравнение функций, о-символика, главная часть функции, порядок малости и порядок роста функции. [Не пройдено]} 
	\subsubsubsection{Критерий Коши существования конечного предела функции. [Не пройдено]} 
	\subsubsubsection{Определения непрерывности функции в точке, их эквивалентность. [Не пройдено]}
	\subsubsubsection{Точки разрыва, их классификация. Непрерывность основных элементарных функций. [Не пройдено]} 
	\subsubsubsection{Арифметические свойства непрерывных функций. [Не пройдено]}
	\subsubsubsection{Теорема о непрерывности сложной функции. [Не пройдено]}
	\subsubsubsection{Теоремы о локальной ограниченности и локальном сохранении знака для функций, непрерывных в точке. [Не пройдено]} 
	\subsubsubsection{Свойства функций, непрерывных на отрезке (первая и вторая теоремы  Вейерштрасса, теорема Коши). Критерий существования и непрерывности обратной функции на промежутке. [Не пройдено]} 
	\subsubsubsection{Понятие равномерной непрерывности функции на множестве. [Не пройдено]} 
	\subsubsubsection{Теорема Кантора [Не пройдено]}
	\subsubsubsection{Система стягивающихся отрезков [В списке вопросов к коллоквиуму отсутствует]} \label{ssss:segments}
	Множество отрезков $\{[a_n; b_n\}_{n=1}^{\infty}$ называется системой стягивающихся отрезков, если выполнено:
	\begin{enumerate}
		\item
		Каждый последовательный отрезок вложен в предыдущий, т.е. $a_1 \ls a_2 \ls ... \ls a_n \ls b_n \ls b_{n - 1} \ls \dots \ls b_1$

		\item
		$\lim\limits_{n \to \infty} (b_n - a_n) = 0$
	\end{enumerate}
	\begin{lemma}
		\textbf{(Коши-Кантора)} Для любой ССТ существует, причем единственная, точка $c$, принадлежащая всем отрезками данной системы, т. е. $\exists! c: \forall n \in \NN \implies c \in [a_n, b_n]$
	\end{lemma}
	\begin{proof}
		\emph{Существование}. Используем аксиому полноты: если $a \ls b$, то $\exists c: a \ls c \ls b$.
		\begin{equation*}
		...
		Тогда $\forall n \in \NN \implies a_n \ls c' < c \ls b_n \implies 0 \ls c - c' \ls b_n - a_n$. Т.к. $\lim\limits_{n \to \infty} (b_n - a_n) = 0 \implies 0 \ls c - c' \ls 0 \implies c - c' = 0 \implies c = c'$
